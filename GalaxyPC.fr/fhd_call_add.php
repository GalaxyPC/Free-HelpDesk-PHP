...
if (isset($_POST['nacl'])){
 if ( $_POST['nacl'] == md5(AUTH_KEY.$db->get_var("select last_login from site_users where user_id = $user_id;")) ) {
	...
	$call_corporate = $db->escape($_POST['call_corporate']);
...
<h4><i class='fa fa-tag'></i> Ajouter un Ticket</h4>
...
<table class="<?php echo $table_style_3;?>" style='width:75%;'>
	...
	<option value='2'>Cloturer</option>
	</select>
	</td>
	</tr>
	...
	<tr>
	<td>Entité</td>
	<td><input type="text" name="call_corporate" class="input-xlarge"></td>
	</tr>
	...
	<tr>
	<td>Lieu pour la demande</td>
	<td><input type="text" name="call_city" class="input-xlarge"></td>
	</tr>
	...
	<tr><td>Service associé</td><td><select name='call_department'>
...
<?php } ?>
	...
	<tr><td>Requète</td><td><select name='call_request'>
...
<?php } ?>
	...
	<tr><td>Produit concerné</td><td><select name='call_device'>
...
<?php } ?>
	...
	<tr>
		<td valign="top">Detail de la demande</td>
...
<?php if (FHD_UPLOAD_ALLOW == "yes") {?>
	<p>Télécharger un fichier: <input type="file" name="hasupload" id="hasupload" size="50"></p>
<?php } ?>
